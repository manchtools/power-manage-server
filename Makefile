...
# The rm is load-bearing. sqlc writes one file per query file but never removes
# outputs whose source is gone, so deleting a queries/*.sql file leaves its
# generated twin behind — still compiling, still referencing a table that may no
# longer exist. That is how a dropped table keeps a live-looking accessor.
# Every file in generated/ carries the "Code generated by sqlc. DO NOT EDIT."
# header, so clearing the directory loses nothing that is not immediately
# rewritten.
sqlc-generate:
	cd internal/store && rm -f generated/*.go && docker run --rm \
