...
-- Seeded with a FIXED timestamp (not NOW()): the emergency rebuild
-- re-applies this exact row after TRUNCATE (rebuild seed in
-- internal/store/rebuild.go — the full-fidelity round-trip test fails
-- if the two drift), and replay must reproduce projections
-- byte-identically (spec 21 AC 6).
INSERT INTO server_settings_projection (id, updated_at)
...
-- permission the app defines; User gets a curated subset. The
-- permission arrays here are only the INITIAL snapshot —
-- auth.ReconcileSystemRoles refreshes them from the code registry on
-- every boot (and after a rebuild). Timestamps are FIXED literals so
-- the rebuild's re-seed (internal/store/rebuild.go) reproduces the
-- rows byte-identically (spec 21 AC 6).
-- ============================================================================
