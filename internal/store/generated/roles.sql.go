...
func (q *Queries) BumpSessionVersionForRoleHolders(ctx context.Context, arg BumpSessionVersionForRoleHoldersParams) (int64, error) {
	result, err := q.db.Exec(ctx, bumpSessionVersionForRoleHolders, arg.RoleID, arg.UpdatedAt)
	if err != nil {
		return 0, err
	}
...
func (q *Queries) CountRoleHolders(ctx context.Context, roleID string) (int64, error) {
	row := q.db.QueryRow(ctx, countRoleHolders, roleID)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
SELECT id, name, description, permissions, is_system, created_at, created_by, updated_at, is_deleted FROM roles WHERE id = $1 AND is_deleted = FALSE
`
...
SELECT id, name, description, permissions, is_system, created_at, created_by, updated_at, is_deleted FROM roles WHERE name = $1 AND is_deleted = FALSE
`
...
func (q *Queries) ListRoles(ctx context.Context, arg ListRolesParams) ([]Role, error) {
	rows, err := q.db.Query(ctx, listRoles, arg.ID, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) SoftDeleteRole(ctx context.Context, arg SoftDeleteRoleParams) (int64, error) {
	result, err := q.db.Exec(ctx, softDeleteRole, arg.ID, arg.UpdatedAt)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
...
func (q *Queries) UpdateSystemRolePermissions(ctx context.Context, arg UpdateSystemRolePermissionsParams) (int64, error) {
	result, err := q.db.Exec(ctx, updateSystemRolePermissions, arg.ID, arg.Permissions, arg.UpdatedAt)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
