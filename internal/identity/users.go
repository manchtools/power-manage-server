...
// DeleteUser erases a subject.
//
// Erasure is a real delete, not a flag: the row, the subject's external
// identity bindings, their group memberships, their role grants, their
// authorized keys and finally their data-encryption key all go in one
// transaction. Destroying the key is what makes every class-three audit
// detail ever sealed for them permanently unreadable, in live rows,
// archives and backups at once.
//
// The audit record for the erasure itself therefore carries NO
// class-three detail: it would be sealed under a key this very
// transaction destroys, and would be born unreadable. It records
// references and a non-reversible digest instead, which is exactly the
// non-personal attribution that must survive.
func (h *Handlers) DeleteUser(ctx context.Context, req *connect.Request[pmv1.DeleteUserRequest]) (*connect.Response[pmv1.DeleteUserResponse], error) {
	if err := h.validate(ctx, req.Msg); err != nil {
		return nil, err
	}
	before, err := h.resolveUserTarget(ctx, PermDeleteUser, req.Msg.Id)
	if err != nil {
		return nil, err
	}
	actor, err := h.requireActor(ctx)
	...
	return connect.NewResponse(&pmv1.DeleteUserResponse{}), nil
}
