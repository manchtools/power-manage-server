...
// Real-PostgreSQL fixture for the identity request boundary.
//
...
const (
	templateDatabase    = "power_manage_identity_test"
...
// database access: a test that needs to seed a subject, plant a grant
// or read the audit tables opens its own connection rather than the
// production type growing an escape hatch for it.
func setupPostgres(t *testing.T) (*store.Store, *pgxpool.Pool) {
	t.Helper()
	sharedOnce.Do(initShared)
	if sharedErr != nil {
		t.Fatalf("identity test fixture: shared postgres init: %v", sharedErr)
	}

	ctx := context.Background()
	dbName := fmt.Sprintf("pm_identity_%d", dbSeq.Add(1))
	...
	createMu.Unlock()
	if err != nil {
		t.Fatalf("identity test fixture: create test database %s: %v", dbName, err)
	}
	...
	t.Cleanup(func() {
		...
		if _, err := shared.admin.ExecContext(dropCtx,
			...
			t.Logf("identity test fixture: drop test database %s: %v", dbName, err)
		}
	})
	...
	if err != nil {
		t.Fatalf("identity test fixture: connect test database %s: %v", dbName, err)
	}
	t.Cleanup(st.Close)

	raw, err := pgxpool.New(ctx, connStr)
	if err != nil {
		t.Fatalf("identity test fixture: open raw pool on %s: %v", dbName, err)
	}
	t.Cleanup(raw.Close)
