...
func NewControlService(st *store.Store, jwtManager *auth.JWTManager, signer ca.ActionSigner, certAuth *ca.CA, gatewayURL string, logger *slog.Logger, enc *crypto.Encryptor, cfg ControlServiceConfig) *ControlService {
	...
	return &ControlService{
		...
		device:           NewDeviceHandler(st, enc, logger.With("component", "device_handler"), signer),
		...
		osquery:          NewOSQueryHandler(st, logger.With("component", "osquery_handler"), signer),
		logs:             NewLogsHandler(st, logger.With("component", "logs_handler"), signer),
