package api_test

import (
	"strings"
	"testing"

	"github.com/manchtools/power-manage/server/internal/api"
)

// TestValidateControlURL covers the shapes the control server may be
// handed via CONTROL_AGENT_URL and the ones registration_handler
// rechecks defensively. Each case captures a real operator footgun
// rc10 reviewers flagged:
//
//   - empty string — rc10 closed the "enroll with empty URL" path.
//   - bare hostname like "agents.example.com" — parses via url.Parse
//     without error but isn't an absolute URL; the agent would dial
//     a relative path and fail with a cryptic error.
//   - http:// — agents refuse h2c as of rc10 (see agent main.go).
//   - ws:// / wss:// — agent uses HTTPS transport for the streaming
//     client; accepting wss here would confuse the operator.
//
// The hostnames below are fixtures for URL SHAPE only; the validator
// never resolves them.
//   - userinfo — credentials in the URL leak on every enrollment
//     response and are never the right answer.
//   - fragment — meaningless on the wire.
func TestValidateControlURL(t *testing.T) {
	cases := []struct {
		name     string
		in       string
		wantErr  bool
		wantWord string // substring that must appear in the error message
	}{
		{"empty", "", true, "empty"},
		{"bare hostname", "agents.example.com", true, "scheme"},
		{"http downgrade", "http://agents.example.com", true, "https"},
		{"wss scheme", "wss://agents.example.com", true, "https"},
		{"ws scheme", "ws://agents.example.com", true, "https"},
		{"userinfo", "https://user:pass@agents.example.com", true, "userinfo"},
		{"userinfo username only", "https://user@agents.example.com", true, "userinfo"},
		{"fragment", "https://agents.example.com#frag", true, "fragment"},
		{"host only", "https://", true, "host"},
		// CR called out that u.Host would accept ":8443" silently
		// (it treats the empty part as a host with a port). Switching
		// to u.Hostname() plus this regression test closes the gap.
		{"port without hostname", "https://:8443", true, "host"},

		{"happy path host", "https://agents.example.com", false, ""},
		{"happy path with port", "https://agents.example.com:8443", false, ""},
		{"happy path with path", "https://agents.example.com/gw/01ABC", false, ""},
		{"happy path with trailing slash", "https://agents.example.com/", false, ""},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			err := api.ValidateControlURL(tc.in)
			if tc.wantErr {
				if err == nil {
					t.Fatalf("ValidateControlURL(%q) = nil, want error containing %q", tc.in, tc.wantWord)
				}
				if tc.wantWord != "" && !strings.Contains(err.Error(), tc.wantWord) {
					t.Errorf("ValidateControlURL(%q) err = %q, want substring %q", tc.in, err.Error(), tc.wantWord)
				}
				return
			}
			if err != nil {
				t.Errorf("ValidateControlURL(%q) = %v, want nil", tc.in, err)
			}
		})
	}
}
