...
var dynamicSQLAllowlist = map[string]string{
	...
	`internal/store/restore.go :: tx.Exec(ctx, "TRUNCATE TABLE "+strings.Join(quoted, ", ")+" CASCADE")`:                                                  "Snapshot restore (spec 19 AC 21): TRUNCATEs the snapshot's own tables before repopulating. The names come from snap.Data keys (produced by snapshotTables → the AllRebuildTargets registry + a pg_tables projection scan), never request input; each is run through pgx.Identifier.Sanitize() and a table identifier cannot be bound as a parameter.",
	"internal/store/restore.go :: tx.Exec(ctx, fmt.Sprintf(`INSERT INTO %s SELECT * FROM jsonb_populate_recordset(NULL::%s, $1::jsonb)`, tbl, tbl), arr)": "Snapshot restore load (spec 19 AC 21): the same registry/pg_tables-derived table name as the TRUNCATE above; not request input, and a table identifier / composite-type name cannot be bound. The row DATA is bound as the $1 jsonb parameter.",
