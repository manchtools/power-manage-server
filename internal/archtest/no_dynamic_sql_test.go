...
var dynamicSQLAllowlist = map[string]string{
	...
	"internal/store/observability.go :: pool.QueryRow(ctx, fmt.Sprintf(` WITH hw AS (SELECT COALESCE(MAX(projection_version), 0) AS v FROM %[1]s) SELECT (SELECT v FROM hw), EXISTS ( SELECT 1 FROM events e WHERE e.stream_type = ANY($1) AND e.sequence_num > (SELECT v FROM hw) AND e.event_type IN ( SELECT DISTINCT h.event_type FROM events h WHERE h.sequence_num IN (SELECT projection_version FROM %[1]s) ) )`, tbl), t.StreamTypes)": "Per-table projection-drift probe (spec 19 AC 31a): `tbl` is one of a target's Tables filtered through the live pg catalog (tablesWithProjectionVersion) — a registry-owned identifier, never request input; a table identifier cannot be bound as a parameter. Stream types ARE bound ($1).",
